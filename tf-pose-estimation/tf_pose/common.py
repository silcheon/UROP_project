...
class CocoPart(Enum): # coco data set에서 지정하는 body part의 key point 값
    Nose = 0
    Neck = 1
    RShoulder = 2
    RElbow = 3
...
class MPIIPart(Enum):
    ...
    @staticmethod
    def from_coco(human): # Coco data set에서 만든 key point 값에 대응되는 MPII data set의 key point 값을 json으로 나타냄. 
        # t = {
        ...
        ### 2D 이미지의 key point 값을 (x, y) 좌표로 나타냄. 
        pose_2d_mpii = [] # key point (x, y) 값 저장
        visibilty = [] # 이미지에 key point가 있는지 없는지 True/False 값으로 나타냄. 
        for mpi, coco in t:
...
CocoPairs = [ # key point들의 연결된 부분을 pair로 나타냄.  
...
CocoColors = [[255, 0, 0], [255, 85, 0], [255, 170, 0], [255, 255, 0], [170, 255, 0], [85, 255, 0], [0, 255, 0],
              ...
              [170, 0, 255], [255, 0, 255], [255, 0, 170], [255, 0, 85]] # keypoint pair를 RGB 값으로 나타냄. pair는 19개 인데 color는 18개인게 Coco data set의 'background' 부분을 제외한 건가 ... ㄴ



def read_imgfile(path, width=None, height=None): # 이미지를 읽어서 사이즈를 재설정해줌. 
